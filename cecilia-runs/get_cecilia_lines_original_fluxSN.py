...
for file in dir:
    # have to re-state the line list every time i think for the o2 mix
    ...
    if (file != "cecilia_2593_3.abund.fits") & (file != "cecilia_2593_35.abund.fits"):  # BX537 doesn't exist in mosfire, MD41 has no nirspec data
        ...
        ######### GET MOSFIRE FLUXES ########
        # idea is to get each mosfire line on the nirspec scale: mos_line * (normalizing_line_nirspec/normalizing_line_mosfire)
        # normalizing line might be ha, hb, or o3. first pref is Ha, because most galaxies have Ha in both detectors


        # first, get G235M data for the individual galaxy
        fluxes235 = hdu[5].data 

        # save flux and error of ha from nirspec
        ha = fluxes235['I_CORR'][lines235['NAME'] == 'Halpha'].item()
        ha_err = fluxes235['I_UNC_CORR'][lines235['NAME'] == 'Halpha'].item()
        # get index of ha in the mosfire data
        ha_ind_mos = (mos['NAMES']=='HAlpha')
        ha_mos = mos['I_CORR'][ha_ind_mos]

        # save flux and error of hb from nirspec
        hb = fluxes235['I_CORR'][lines235['NAME'] == 'Hbeta'].item()
        hb_err = fluxes235['I_UNC_CORR'][lines235['NAME'] == 'Hbeta'].item()
        # get index of hb in the mosfire data
        ...
        if (ha_mos != 0) & (ha != 0):
            ...
            # to get the error on the luminosity, multiply the luminosity by the fractional uncertainty from the flux 
            fractional_flux_uncertainty = np.nan_to_num(mos_flux_err/mos_flux)
            mos_lum_err = mos_lum * fractional_flux_uncertainty 

            # Record
            for j, line in enumerate(mos_lum):
                if line > mos_lum_err[j]: # if flux is greater than error, save as detected line
                ...
                else: # save as upper limit
                    line_lum_up[line_list == cat_to_cue_mosfire[in_cue_mos[j]]] = float(mos_lum_err[j]) * 3
                    line_lum_unc[line_list == cat_to_cue_mosfire[in_cue_mos[j]]] = float(mos_lum_err[j])
         

        elif (hb_mos != 0) & (hb != 0):
            # from when I had to check what all the lines were being normalized with 
        ...
        ########### GET G235M FLUXES ##############
        # if line is detected in g235m, basically overwrite mosfire fluxes
        ...
        print("g235 i_corr and i_unc_corr", fluxes235['I_CORR'][to_use_235], fluxes235['I_UNC_CORR'][to_use_235])
        ...
        print("g235 flux and f_unc", rawlines235["FLUX"][to_use_235], rawlines235["F_UNC"][to_use_235])
        # to get the error on the luminosity, multiply the luminosity by the fractional uncertainty from the flux 
        fractional_flux_uncertainty = np.nan_to_num(flux_err_235/flux_235)
        lum_err_235 = lum_235 * fractional_flux_uncertainty

        #print("g235m lum err", np.nan_to_num(flux_err_235/flux_235), flux_err_235/flux_235)
        ...
        for j, line in enumerate(lum_235):
            if line > lum_err_235[j]:
                ...
                if line_lum[line_list == cat_to_cue_g235m[in_cue_235[j]]] == 0:  
                    # don't overwrite a line luminosity as an upper limit if the line is detected in mosfire but not in g235m.                                                                                       # honestly not sure why it wouldn't be though
                    line_lum_unc[line_list == cat_to_cue_g235m[in_cue_235[j]]] = float(lum_err_235[j])
        
        ############### GET G395M FLUXES  ###########
        # if line is detected in g395m, compare to g235m SN then save the higher-SN line detection
        ...
        # to get the error on the luminosity, multiply the luminosity by the fractional uncertainty from the flux 
        # fractional_flux_uncertainty = np.nan_to_num(flux_err_395/flux_395)
        # lum_err_395 = lum_395 * fractional_flux_uncertainty

        """issue: in the original flux_SN run, I hadn't changed any code for g395m so it was using the original SN. so I'm also doing that too"""
        ...
        nans = np.isnan(dic['line_lum_unc'])
        print("lines where uncertainty is nan...", dic['line_name'][nans])
        print("flux, upper lim of these lines", dic['line_lum'][nans], dic['line_lum_up'][nans])
