...
for galaxy in galaxies:
    ...
    linelum_dir = "galaxy-observational-data/line_luminosities_dec/"
    results_dir = "cue-results/dec-run/"
    figures_dir = "figures/dec-run/"
    linepred_dir = "line-predictions/dec-run/"
    ...
    endtag += "_oIIsum"
    OII_ind, = np.where((obs['line_name'] == 'O  2 3726.03A') | (obs['line_name'] == 'O  2 3728.81A'))
    ...
    # new: edit the line noise array so it's the same length as the delta array with the summed o2 lines instead of the individual o2 lines
    line_noise_OII = np.sum(np.sqrt(line_noise[OII_ind][0]**2+line_noise[OII_ind][1]**2))  # or should I be adding it in quadrature?
    wav_ind_no_o2 = np.setdiff1d(wav_ind, OII_ind)
    line_noise = np.concatenate([line_noise[wav_ind_no_o2], [line_noise_OII]])
    ...
    def loglike_cue(model_params):
        ...
        delta[wav_ind_obs] = obs['line_lum'][wav_ind_obs]-fit[wav_ind_obs]   # calculate the difference between obs and model at the indices where the lines are observed
        delta[wav_ind_up] = 0-fit[wav_ind_up]
        # now, modify the delta function to instead compare the sum of the o2 lines
        
          # get indices where the individual o2 lines are
        delta[wav_ind_no_o2] = obs['line_lum'][wav_ind_no_o2]-fit[wav_ind_no_o2]  # get the delta array for all obs lines but individual o1
        line_lum_OII = np.sum(obs['line_lum'][OII_ind])
        delta_OII = line_lum_OII - np.sum(fit[OII_ind])  
        delta = np.concatenate([delta[wav_ind_no_o2], [delta_OII]]) # add the delta of the sum of the o2 lines to the delta array with individual o2 taken out

        lnp = -0.5*( (delta**2/line_var) + np.log(2*np.pi*line_var))
                #-0.5*( (delta**2/line_var[wav_ind]) + np.log(2*np.pi*line_var[wav_ind]) )
    ...
    with open(results_dir+galaxy+"_results_"+endtag+"_stellar.pkl", 'wb') as f:
    ...
    plt.savefig(figures_dir+galaxy+"_cornerplot_"+endtag+"_stellar.png")
    ...
    with open(linepred_dir+galaxy+"_"+endtag+".pkl", 'wb') as f:
    ...
    plt.tight_layout()
    plt.savefig(figures_dir+galaxy+"_lineprediction_"+endtag+"_stellar.png")